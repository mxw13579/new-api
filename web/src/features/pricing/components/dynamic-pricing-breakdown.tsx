...
type DynamicPricingBreakdownPlan = Omit<StablePricingPlan, 'viewModel'> & {
  tiers: BreakdownTier[]
  ruleGroups: RequestRuleGroup[]
  viewModel: {
    tiers: Array<KeyedPricingItem<BreakdownTier>>
    ruleGroups: Array<KeyedPricingItem<RequestRuleGroup>>
  }
}

/** 为所有定价列表统一解析内容并规划稳定的行标识。 */
function planDynamicPricingBreakdown(
  ...
  const tiers: BreakdownTier[] = usageSchema
    ...
    if (compiled.status === 'ready') {
      parsedRules = requestRuleGroupsFromTrace(
        compiled.requestRules.map((rule) => ({
          cond: expression.slice(rule.condition.start, rule.condition.end),
  ...
  const ruleGroups = parsedRules || []
  const plan = coordinator.plan({
    // 协调器只消费内容指纹；保留完整任务档位，避免条件变化丢失身份信息。
    tiers: tiers.map((tier) =>
      isTaskBreakdownTier(tier)
        ? { label: tier.label, conditions: [], taskTier: tier }
        : tier
    ),
    ruleGroups,
  })
  return {
    ...
    viewModel: {
      tiers: tiers.map((tier, index) => ({
        item: tier,
        key: plan.viewModel.tiers[index].key,
      })),
      ruleGroups: plan.viewModel.ruleGroups,
    },
  }
}
...
export function DynamicPricingBreakdown({
  ...
  const pricingPlan = useMemo(
    () =>
      planDynamicPricingBreakdown(
        pricingCoordinator,
        expr,
        requestRules,
        usageSchema
      ),
    [expr, pricingCoordinator, requestRules, usageSchema]
  )
  useLayoutEffect(() => {
          ...
          <StaticDataTable
            ...
            columns={[
              ...
              ...visiblePriceFields.map((field) => ({
                ...
                cell: (keyedTier: KeyedPricingItem<BreakdownTier>) => {
                  const value = field.value(keyedTier.item)
                  return value > 0 ||
