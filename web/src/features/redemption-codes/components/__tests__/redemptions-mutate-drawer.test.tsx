...
const isolatedRun = process.env.REDEMPTION_DRAWER_DOM_TEST_ISOLATED === '1'

if (!isolatedRun) {
  test('redemption drawer contracts run in a disposable Bun process', () => {
    const child = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          REDEMPTION_DRAWER_DOM_TEST_ISOLATED: '1',
        },
        encoding: 'utf8',
        timeout: 20_000,
      }
    ...
    assert.equal(
      output.includes('Maximum update depth exceeded'),
      false,
      output
    )
  })
  ...
  const { RedemptionsMutateDrawer } =
    await import('../redemptions-mutate-drawer')
  ...
  test('redemption drawer blocks updates and uses localized feedback for unsuccessful responses', async () => {
    ...
    assert.equal(getSaveButton().disabled, true)
    assert.equal(
      document.body.textContent?.includes('raw server message'),
      false
    )
  })
