...
describe('invoice admin contracts', () => {
  ...
  test('removes sensitive values instead of merely hiding them with CSS', () => {
    ...
    assert.doesNotMatch(
      ...
      /Secret Company|91310000SECRET|11010519491231002X/
    )
    assert.equal(
      baseDetail.profile_snapshot.identity_card_number,
      '11010519491231002X'
    )
    assert.equal(
    ...
    assert.equal(
      maskInvoiceSensitiveDetail(baseDetail, true).profile_snapshot
