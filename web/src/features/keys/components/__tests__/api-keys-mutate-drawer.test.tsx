...
const { afterAll, afterEach } = (await import('bun:test')) as unknown as {
...
if (!isolatedRun) {
  describe('API keys mutate drawer isolation', () => {
    test('verifies the drawer contracts in a disposable Bun process', () => {
  ...
  describe('API keys mutate drawer Auto group integration', () => {
    ...
    test('preserves an unsaved custom order and mode after Auto to ordinary to Auto changes', async () => {
      ...
      const autoOrderControl =
        getControlByLabel<HTMLElement>('Auto group order')
