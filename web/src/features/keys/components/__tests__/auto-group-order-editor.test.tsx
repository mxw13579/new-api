...
const { afterAll } = (await import('bun:test')) as unknown as {
  afterAll(callback: () => void | Promise<void>): void
}
const isolatedRun = process.env.AUTO_GROUP_DOM_TEST_ISOLATED === '1'

if (!isolatedRun) {
  describe('Auto group order editor isolation', () => {
    test('verifies the editor contracts in a disposable Bun process', () => {
      const child = spawnSync(
        process.execPath,
        [
          'test',
          'src/features/keys/components/__tests__/auto-group-order-editor.test.tsx',
        ],
        {
          cwd: process.cwd(),
          env: { ...process.env, AUTO_GROUP_DOM_TEST_ISOLATED: '1' },
          encoding: 'utf8',
          timeout: 15_000,
        }
      )
  ...
  describe('Auto group order editor', () => {
    ...
    test('enforces the limit and exposes accessible reorder controls', async () => {
      ...
      assert.equal(
        container.textContent?.includes('2 / 2 groups selected'),
        true
      )
      assert.ok(
    ...
    test('removing the last custom group does not silently enable inheritance', async () => {
      ...
      await act(async () =>
        root.render(<Harness initialGroups={['default']} />)
      )
