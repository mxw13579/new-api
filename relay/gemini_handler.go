...
func getGeminiInputTokens(req *gemini.GeminiChatRequest, info *relaycommon.RelayInfo) int {
	...
	input, err := service.CountTokenInput(inputText, info.UpstreamModelName)
	if err != nil {
		return 0
	}
	inputTokens := input
...
func GeminiHelper(c *gin.Context) (openaiErr *dto.OpenAIErrorWithStatusCode) {
	...
	if err != nil {
		...
		return service.OpenAIErrorWrapperLocal(err, "invalid_request", http.StatusInternalServerError)
	}
	...
	if setting.ShouldCheckPromptSensitive() {
		...
		if err != nil {
			...
			return service.OpenAIErrorWrapperLocal(err, "sensitive_words_detected", http.StatusInternalServerError)
		}
	}

	// model mapped 模型映射
	err = helper.ModelMappedHelperG(c, relayInfo, req)
	if err != nil {
		return service.OpenAIErrorWrapperLocal(err, "channel:model_mapped_error", http.StatusInternalServerError)
	}
	...
	if adaptor == nil {
		return service.OpenAIErrorWrapperLocal(err, "invalid_api_type", http.StatusInternalServerError)
	}
