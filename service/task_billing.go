...
func RefundTaskQuota(ctx context.Context, task *model.Task, reason string) bool {
	...
	// Roll back accumulated usage once, after the durable refund winner is known.
	model.UpdateUserUsedQuota(refundedTask.UserId, -quota)
	model.UpdateChannelUsedQuota(refundedTask.ChannelId, -quota)

	// 3. 记录日志
	other := taskBillingOther(refundedTask)
	other.SetPublic("task_id", refundedTask.TaskID)
