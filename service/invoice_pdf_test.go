...
func TestValidateInvoicePDFRejectsDamagedEncryptedAndActiveContent(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "open action", catalogExtra: "/OpenAction 5 0 R", extra: []string{"<< /S /GoTo /D [3 0 R /Fit] >>"}},
		{name: "additional action", catalogExtra: "/AA << /WC 5 0 R >>", extra: []string{"<< /S /GoTo /D [3 0 R /Fit] >>"}},
