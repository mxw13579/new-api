		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, upgrade := range []bool{false, true} {
				t.Run(fmt.Sprintf("upgrade=%v", upgrade), func(t *testing.T) {
					...
					if upgrade {
						require.NoError(t, db.AutoMigrate(&releasedAuditUser{}, &releasedAuditLog{}, &model.Option{}))
						require.NoError(t, db.Create(&model.Option{Key: "invoice_setting.minimum_amount_minor", Value: "0"}).Error)
					...
					if upgrade {
						...
						var invoiceOption model.Option
						require.NoError(t, db.Where(map[string]any{"key": "invoice_setting.minimum_amount_minor"}).Take(&invoiceOption).Error)
						assert.Equal(t, "10000", invoiceOption.Value)
					}
